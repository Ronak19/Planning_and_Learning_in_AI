...
class Particle(object):
    ...
    def orient(self):
        return [(self.pos_x, self.pos_y), (self.pos_x + self.orient_length * math.cos(self.orientation), self.pos_y + self.orient_length * math.sin(self.orientation))]

    def update(self, obs, dyn):
        """After the motion, update the weight of the particle and its EKFs based on the sensor data"""
        for o in obs:
            prob = np.exp(-70)
            #if dyn[obs.index(o)] == 1:
                #self.landmarks = []
            if self.landmarks:
                # find the data association with ML
                prob, landmark_idx, ass_obs, ass_jacobian, ass_adjcov = self.find_data_association(o)
                #print('predicted obs: ', ass_obs)
                self.lm_error += euclidean_distance(ass_obs, o)
                if prob < self.TOL or self.landmarks[landmark_idx].dynamic() == 1:
                    if self.landmarks[landmark_idx].dynamic() == 1:
                        self.landmarks.pop(landmark_idx)
                    # create new landmark
                    self.create_landmark(o, dyn[obs.index(o)])
                else:
                ...
                self.create_landmark(o, dyn[obs.index(o)])
            self.weight *= prob
            self.lm_error /= len(obs)


    def sense(self, landmarks, radius):
        """
        Only for robot.
        ...
        for i in range(len(landmarks)):
            ...
            dis1 = euclidean_distance(l, (self.pos_x, self.pos_y))
            if dis1 < radius:
                ...
                dyn.append(landmarks[i].dynamic())
    ...
    def create_landmark(self, obs, dyn):
        landmark = self.guess_landmark(obs, dyn)
